...
# Global session for cinstagram to reuse connections
...
def cinstagram(email, token, chat_id):
    ...
    try:
        resp = cinstagram_session.post(CONFIG["insta_recovery"], headers=headers, data=data).text
        if email in resp:
            good_insta += 1
...
def iaccount(username, domain, token, chat_id):
    ...
    if followers < min_followers:
        return  
    hits += 1  # Takipçi filtresinden geçti, hits artırılır
